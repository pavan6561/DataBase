...
class RpcBaseHeader
{
  public:
    ...
    static constexpr int64_t START_METHOD_ID{0};
    ...
    /// @briet Obtains the method ID of the RPC message
    /// @return the methodId of the RPC message
    int64_t getMethodId() const noexcept;
...
class RequestHeader : public RpcBaseHeader
{
  public:
    ...
    /// @brief Sets the method ID which is used to match a response to a request
    /// @param[in] methodId is to identify the method call.
    void setMethodId(const int64_t methodId) noexcept;
...
class ResponseHeader : public RpcBaseHeader
{
  public:
    ...
    explicit ResponseHeader(const cxx::UniqueId& uniqueClientQueueId,
                            const uint32_t lastKnownClientQueueIndex,
                            const int64_t sequenceId,
                            const int64_t methodId) noexcept;
