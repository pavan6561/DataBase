...
namespace runtime
{
PoshRuntimeImpl::PoshRuntimeImpl(
    cxx::optional<const RuntimeName_t*> name, const RuntimeLocation location) noexcept :
    PoshRuntime(name),
    m_ipcChannelInterface(roudi::IPC_CHANNEL_ROUDI_NAME, *name.value(),
        iox::version::VersionInfo::getCurrentVersion(), runtime::PROCESS_WAITING_FOR_ROUDI_TIMEOUT),
    m_ShmInterface([&] {
      ...
      return location == RuntimeLocation::SAME_PROCESS_LIKE_ROUDI
          ? cxx::nullopt
          : cxx::optional<SharedMemoryUser>(
                {m_ipcChannelInterface.getShmTopicSize(), m_ipcChannelInterface.getSegmentId(),
...
PoshRuntimeImpl::~PoshRuntimeImpl() noexcept
{
  ...
  if (m_ipcChannelInterface.sendRequestToDaemeon(sendBuffer, receiveBuffer)
      && (1U == receiveBuffer.getNumberOfElements()))
  {
    std::string IpcMessage = receiveBuffer.getElementAtIndex(0U);

    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::TERMINATION_ACK)
    {
...
PublisherPortUserType::MemberType_t* PoshRuntimeImpl::getMiddlewarePublisher(
    const capro::ServiceDescription& service, const popo::PublisherOptions& publisherOptions,
    const PortConfigInfo& portConfigInfo) noexcept
{
  constexpr uint64_t MAX_HISTORY_CAPACITY = PublisherPortUserType::MemberType_t::ChunkSenderData_t::
  ...
  if (maybePublisher.has_error())
  {
    ...
    case IpcMessageErrorType::REQUEST_PUBLISHER_INVALID_RESPONSE:
      ...
      errorHandler(
          PoshError::POSH__RUNTIME_ROUDI_REQUEST_PUBLISHER_INVALID_RESPONSE, iox::ErrorLevel::SEVERE);
      break;
    ...
    PoshRuntimeImpl::requestPublisherFromRoudi(const IpcMessage& sendBuffer) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_PUBLISHER_ACK)
  ...
  else if (receiveBuffer.getNumberOfElements() == 2U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage1.c_str()) == IpcMessageType::ERROR)
    {
      LogError() << "Request publisher received no valid publisher port from RouDi.";
      return cxx::error<IpcMessageErrorType>(
          stringToIpcMessageErrorType<IpcMessageErrorType>(IpcMessage2.c_str()));
    }
  }
...
SubscriberPortUserType::MemberType_t* PoshRuntimeImpl::getMiddlewareSubscriber(
    const capro::ServiceDescription& service, const popo::SubscriberOptions& subscriberOptions,
    const PortConfigInfo& portConfigInfo) noexcept
{
  constexpr uint64_t MAX_QUEUE_CAPACITY
      = SubscriberPortUserType::MemberType_t::ChunkQueueData_t::MAX_CAPACITY;
  ...
  if (maybeSubscriber.has_error())
  {
    ...
    case IpcMessageErrorType::REQUEST_SUBSCRIBER_INVALID_RESPONSE:
      ...
      errorHandler(
          PoshError::POSH__RUNTIME_ROUDI_REQUEST_SUBSCRIBER_INVALID_RESPONSE, iox::ErrorLevel::SEVERE);
      break;
    ...
    PoshRuntimeImpl::requestSubscriberFromRoudi(const IpcMessage& sendBuffer) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_SUBSCRIBER_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
...
popo::ClientPortUser::MemberType_t* PoshRuntimeImpl::getMiddlewareClient(
    ...
    const PortConfigInfo& portConfigInfo) noexcept
{
  ...
  if (maybeClient.has_error())
  {
    ...
    case IpcMessageErrorType::REQUEST_CLIENT_WRONG_IPC_MESSAGE_RESPONSE:
      ...
      errorHandler(
          PoshError::POSH__RUNTIME_ROUDI_REQUEST_CLIENT_WRONG_IPC_MESSAGE_RESPONSE, iox::ErrorLevel::SEVERE);
      break;
    ...
    PoshRuntimeImpl::requestClientFromRoudi(const IpcMessage& sendBuffer) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_CLIENT_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
...
popo::ServerPortUser::MemberType_t* PoshRuntimeImpl::getMiddlewareServer(
  ...
  if (maybeServer.has_error())
  {
    ...
    case IpcMessageErrorType::REQUEST_SERVER_WRONG_IPC_MESSAGE_RESPONSE:
      LogWarn() << "Could not create server with service description '" << service
                << "'; received wrong IPC channel response.";
      errorHandler(
          PoshError::POSH__RUNTIME_ROUDI_REQUEST_SERVER_WRONG_IPC_MESSAGE_RESPONSE, iox::ErrorLevel::SEVERE);
      break;
    ...
    PoshRuntimeImpl::requestServerFromRoudi(const IpcMessage& sendBuffer) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_SERVER_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
...
popo::InterfacePortData* PoshRuntimeImpl::getMiddlewareInterface(
    const capro::Interfaces interface, const NodeName_t& nodeName) noexcept
{
  IpcMessage sendBuffer;
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_INTERFACE_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
  ...
  LogError() << "Get mw interface got wrong response from IPC channel :'" << receiveBuffer.getMessage()
             << "'";
  errorHandler(
      PoshError::POSH__RUNTIME_ROUDI_GET_MW_INTERFACE_WRONG_IPC_MESSAGE_RESPONSE, iox::ErrorLevel::SEVERE);
  return nullptr;
}
...
NodeData* PoshRuntimeImpl::createNode(const NodeProperty& nodeProperty) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str()) == IpcMessageType::CREATE_NODE_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
  ...
  errorHandler(
      PoshError::POSH__RUNTIME_ROUDI_CREATE_NODE_WRONG_IPC_MESSAGE_RESPONSE, iox::ErrorLevel::SEVERE);
  return nullptr;
}
    ...
    PoshRuntimeImpl::requestConditionVariableFromRoudi(const IpcMessage& sendBuffer) noexcept
{
  ...
  else if (receiveBuffer.getNumberOfElements() == 3U)
  {
    ...
    if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str())
        == IpcMessageType::CREATE_CONDITION_VARIABLE_ACK)
    {
      rp::BaseRelativePointer::id_t segmentId{0U};
      cxx::convert::fromString(receiveBuffer.getElementAtIndex(2U).c_str(), segmentId);
      rp::BaseRelativePointer::offset_t offset{0U};
  ...
  LogError() << "Request condition variable got wrong response from IPC channel :'"
             << receiveBuffer.getMessage() << "'";
  return cxx::error<IpcMessageErrorType>(
      IpcMessageErrorType::REQUEST_CONDITION_VARIABLE_WRONG_IPC_MESSAGE_RESPONSE);
}
...
popo::ConditionVariableData* PoshRuntimeImpl::getMiddlewareConditionVariable() noexcept
{
  ...
  if (maybeConditionVariable.has_error())
  {
    ...
    default:
      ...
      errorHandler(
          PoshError::POSH__RUNTIME_ROUDI_CONDITION_VARIABLE_CREATION_UNKNOWN_ERROR, iox::ErrorLevel::SEVERE);
      break;
    }
    return nullptr;
  }
...
bool PoshRuntimeImpl::sendRequestToRouDi(const IpcMessage& msg, IpcMessage& answer) noexcept
{
  ...
  return m_ipcChannelInterface.sendRequestToDaemeon(msg, answer);
}
...
void PoshRuntimeImpl::sendKeepAliveAndHandleShutdownPreparation() noexcept
{
  ...
  if (m_shutdownRequested.exchange(false, std::memory_order_relaxed))
  {
        ...
        && (1U == receiveBuffer.getNumberOfElements()))
    {
      ...
      if (stringToIpcMessageType<IpcMessageType>(IpcMessage.c_str())
          == IpcMessageType::PREPARE_APP_TERMINATION_ACK)
      {
