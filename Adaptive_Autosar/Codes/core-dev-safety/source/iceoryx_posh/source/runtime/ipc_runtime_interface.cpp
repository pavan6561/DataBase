...
namespace runtime
{
IpcRuntimeInterface::IpcRuntimeInterface(const RuntimeName_t& roudiName, const RuntimeName_t& runtimeName,
    const iox::version::VersionInfo& versionInfo, const units::Duration roudiWaitingTimeout) noexcept :
    m_runtimeName(runtimeName),
    m_AppIpcInterface(runtimeName), m_RoudiIpcInterface(roudiName), m_versionInfo(versionInfo)
{
  ...
  while (!timer.hasExpired() && regState != RegState::FINISHED)
  {
    ...
    case RegState::SEND_REGISTER_REQUEST:
    {
      ...
      int pid = getpid();
      cxx::Expects(pid >= 0);
      sendBuffer << IpcMessageTypeToString<IpcMessageType>(IpcMessageType::REG) << m_runtimeName
                 << cxx::convert::toString(pid)
...
bool IpcRuntimeInterface::sendRequestToDaemeon(const IpcMessage& msg, IpcMessage& answer) noexcept
{
...
IpcRuntimeInterface::RegAckResult IpcRuntimeInterface::waitForRegAck(int64_t transmissionTimestamp) noexcept
{
  ...
  while (retryCounter++ < MAX_RETRY_COUNT)
  {
    ...
    if (m_AppIpcInterface.timedReceive(1_s, receiveBuffer))
    {
      ...
      if (stringToIpcMessageType<IpcMessageType>(cmd.c_str()) == IpcMessageType::REG_ACK)
      {
