...
namespace runtime
{
IpcInterfaceCreator::IpcInterfaceCreator(
    const RuntimeName_t& runtimeName, const uint64_t maxMessages, const uint64_t messageSize) noexcept :
        ...
        posix::FileLock::create(runtimeName)
            .or_else([&runtimeName](auto& error) {
              if (error == posix::FileLockError::LOCKED_BY_OTHER_PROCESS)
              {
                ...
                errorHandler(
                    PoshError::IPC_INTERFACE__APP_WITH_SAME_NAME_STILL_RUNNING, iox::ErrorLevel::FATAL);
              }
              else
              {
