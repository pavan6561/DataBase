...
ChunkHeader::ChunkHeader(const uint32_t chunkSize, const ChunkSettings& chunkSettings) noexcept :
    m_chunkSize(chunkSize), m_userHeaderSize(chunkSettings.userHeaderSize()),
    m_userPayloadSize(chunkSettings.userPayloadSize()),
    m_userPayloadAlignment(chunkSettings.userPayloadAlignment())
{
  static_assert(
      alignof(ChunkHeader) >= 8U, "All the calculations expect the ChunkHeader alignment to be at least 8!");
  ...
  cxx::Ensures(
      overflowSafeUsedSizeOfChunk() <= chunkSize && "Used size of chunk would exceed the actual chunk size!");
}
