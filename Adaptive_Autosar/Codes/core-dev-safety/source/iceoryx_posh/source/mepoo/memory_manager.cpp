...
void MemoryManager::addMemPool(posix::Allocator& managementAllocator, posix::Allocator& chunkMemoryAllocator,
...
void MemoryManager::generateChunkManagementPool(posix::Allocator& managementAllocator) noexcept
{
  ...
  m_chunkManagementPool.emplace_back(
      chunkSize, m_totalNumberOfChunks, managementAllocator, managementAllocator);
}
...
uint64_t MemoryManager::requiredManagementMemorySize(const MePooConfig& mePooConfig) noexcept
{
  ...
  for (const auto& mempool : mePooConfig.m_mempoolConfig)
  {
    ...
    memorySize += cxx::align(
        static_cast<uint64_t>(MemPool::freeList_t::requiredIndexMemorySize(mempool.m_chunkCount)),
        MemPool::CHUNK_MEMORY_ALIGNMENT);
  }

  memorySize += cxx::align(
      static_cast<uint64_t>(sumOfAllChunks * sizeof(ChunkManagement)), MemPool::CHUNK_MEMORY_ALIGNMENT);
  memorySize
      += cxx::align(static_cast<uint64_t>(MemPool::freeList_t::requiredIndexMemorySize(sumOfAllChunks)),
          MemPool::CHUNK_MEMORY_ALIGNMENT);
...
void MemoryManager::configureMemoryManager(const MePooConfig& mePooConfig,
    posix::Allocator& managementAllocator, posix::Allocator& chunkMemoryAllocator) noexcept
{
...
cxx::expected<SharedChunk, MemoryManager::Error> MemoryManager::getChunk(
