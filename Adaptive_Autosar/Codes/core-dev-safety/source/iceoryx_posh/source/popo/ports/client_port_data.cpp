...
{
  return policy == QueueFullPolicy::DISCARD_OLDEST_DATA
...
ClientPortData::ClientPortData(const capro::ServiceDescription& serviceDescription,
    const RuntimeName_t& runtimeName, const ClientOptions& clientOptions,
    mepoo::MemoryManager* const memoryManager, const mepoo::MemoryInfo& memoryInfo) noexcept :
    ...
    m_chunkReceiverData(getResponseQueueType(clientOptions.responseQueueFullPolicy),
        clientOptions.responseQueueFullPolicy, memoryInfo),
