...
ServerPortUser::ServerPortUser(MemberType_t& serverPortData) noexcept :
    BasePort(&serverPortData), m_chunkSender(&getMembers()->m_chunkSenderData),
...
cxx::expected<const RequestHeader*, ServerRequestResult> ServerPortUser::getRequest() noexcept
{
  ...
  if (getChunkResult.has_error())
  {
    if (!isOffered())
    {
      return cxx::error<ServerRequestResult>(
  ...
  return cxx::success<const RequestHeader*>(
      static_cast<const RequestHeader*>(getChunkResult.value()->userHeader()));
}
...
cxx::expected<ResponseHeader*, AllocationError> ServerPortUser::allocateResponse(
    const RequestHeader* const requestHeader, const uint32_t userPayloadSize,
    const uint32_t userPayloadAlignment) noexcept
{
  ...
  auto* responseHeader = new (allocateResult.value()->userHeader())
      ResponseHeader(requestHeader->m_uniqueClientQueueId, requestHeader->m_lastKnownClientQueueIndex,
          requestHeader->getSequenceId(), requestHeader->getMethodId());
...
cxx::expected<ServerSendError> ServerPortUser::sendResponse(ResponseHeader* const responseHeader) noexcept
{
  ...
  m_chunkSender
      .getQueueIndex(responseHeader->m_uniqueClientQueueId, responseHeader->m_lastKnownClientQueueIndex)
...
void ServerPortUser::setConditionVariable(
