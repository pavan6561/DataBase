...
{
  return cxx::Serialization::create(historyCapacity, nodeName, offerOnCreate,
      static_cast<std::underlying_type_t<ConsumerTooSlowPolicy>>(subscriberTooSlowPolicy));
}

cxx::expected<PublisherOptions, cxx::Serialization::Error> PublisherOptions::deserialize(
    const cxx::Serialization& serialized) noexcept
{
  ...
  auto deserializationSuccessful = serialized.extract(publisherOptions.historyCapacity,
      publisherOptions.nodeName, publisherOptions.offerOnCreate, subscriberTooSlowPolicy);
