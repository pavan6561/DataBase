...
Listener::Listener(ConditionVariableData& conditionVariableData) noexcept : Parent(conditionVariableData)
{
}
...
bool Event_t::init(const uint64_t eventId, void* const origin, void* const userType, const uint64_t eventType,
...
bool Event_t::isEqualTo(
    const void* const origin, const uint64_t eventType, const uint64_t eventTypeHash) const noexcept
{
...
{
  return m_origin != nullptr && m_eventId != INVALID_ID && m_eventType != INVALID_ID
      && m_eventTypeHash != INVALID_ID && m_callback != nullptr && m_translationCallback != nullptr
      && m_invalidationCallback != cxx::MethodCallback<void, uint64_t>();
}
} // namespace internal
