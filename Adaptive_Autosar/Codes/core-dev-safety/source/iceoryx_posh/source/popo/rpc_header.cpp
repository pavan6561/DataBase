...
RpcBaseHeader::RpcBaseHeader(const cxx::UniqueId& uniqueClientQueueId,
    const uint32_t lastKnownClientQueueIndex, const int64_t sequenceId, const int64_t methodId,
    const uint8_t rpcHeaderVersion) noexcept :
    m_rpcHeaderVersion(rpcHeaderVersion),
    m_lastKnownClientQueueIndex(lastKnownClientQueueIndex), m_uniqueClientQueueId(uniqueClientQueueId),
    m_sequenceId(sequenceId), m_methodId(methodId)
{
}
...
void RequestHeader::setMethodId(const int64_t methodId) noexcept
{
  this->m_methodId = methodId;
}

int64_t RpcBaseHeader::getMethodId() const noexcept
{
  return m_methodId;
}
...
RequestHeader::RequestHeader(
    const cxx::UniqueId& uniqueClientQueueId, const uint32_t lastKnownClientQueueIndex) noexcept :
    RpcBaseHeader(uniqueClientQueueId, lastKnownClientQueueIndex, START_SEQUENCE_ID, START_METHOD_ID,
        RPC_HEADER_VERSION)
{
}
...
ResponseHeader::ResponseHeader(const cxx::UniqueId& uniqueClientQueueId,
    const uint32_t lastKnownClientQueueIndex, const int64_t sequenceId, const int64_t methodId) noexcept :
    RpcBaseHeader(uniqueClientQueueId, lastKnownClientQueueIndex, sequenceId, methodId, RPC_HEADER_VERSION)
{
}
