...
TriggerHandle::TriggerHandle(ConditionVariableData& conditionVariableData,
    const cxx::MethodCallback<void, uint64_t> resetCallback, const uint64_t uniqueTriggerId) noexcept :
    m_conditionVariableDataPtr(&conditionVariableData),
    m_resetCallback(resetCallback), m_uniqueTriggerId(uniqueTriggerId)
{
  if (!m_resetCallback)
  {
...
bool TriggerHandle::wasTriggered() const noexcept
{
  ...
  {
    return m_conditionVariableDataPtr->m_activeNotifications[m_uniqueTriggerId].load(
