...
{
  return cxx::Serialization::create(queueCapacity, historyRequest, nodeName, subscribeOnCreate,
      static_cast<std::underlying_type_t<QueueFullPolicy>>(queueFullPolicy), requiresPublisherHistorySupport);
}

cxx::expected<SubscriberOptions, cxx::Serialization::Error> SubscriberOptions::deserialize(
    const cxx::Serialization& serialized) noexcept
{
  ...
  auto deserializationSuccessful = serialized.extract(subscriberOptions.queueCapacity,
      subscriberOptions.historyRequest, subscriberOptions.nodeName, subscriberOptions.subscribeOnCreate,
      queueFullPolicy, subscriberOptions.requiresPublisherHistorySupport);
