...
{
  return cxx::Serialization::create(requestQueueCapacity, nodeName, offerOnCreate,
...
cxx::expected<ServerOptions, cxx::Serialization::Error> ServerOptions::deserialize(
    const cxx::Serialization& serialized) noexcept
{
  ...
  auto deserializationSuccessful = serialized.extract(serverOptions.requestQueueCapacity,
      serverOptions.nodeName, serverOptions.offerOnCreate, requestQueueFullPolicy, clientTooSlowPolicy);
