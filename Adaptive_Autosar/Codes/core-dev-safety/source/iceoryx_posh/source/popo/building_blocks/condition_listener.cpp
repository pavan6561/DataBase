  ...
  while (!hasFatalError
      && getMembers()
             ...
             .or_else([&](posix::SemaphoreError) {
               errorHandler(
                   PoshError::POPO__CONDITION_LISTENER_SEMAPHORE_CORRUPTED_IN_RESET, ErrorLevel::FATAL);
               hasFatalError = true;
             })
             .value())
  {
...
ConditionListener::NotificationVector_t ConditionListener::wait(std::unique_lock<std::mutex>& lock) noexcept
{
  lock.unlock();
...
ConditionListener::NotificationVector_t ConditionListener::timedWait(
    const units::Duration& timeToWait) noexcept
{
  std::cout << __func__ << " TimedWait - " << timeToWait << "\n\n";
...
ConditionListener::NotificationVector_t ConditionListener::timedWait(
    std::unique_lock<std::mutex>& lock, const units::Duration& timeToWait) noexcept
{
  std::cout << __func__ << " TimedWait - " << timeToWait << "\n\n";
  lock.unlock();
...
ConditionListener::NotificationVector_t ConditionListener::waitImpl(
    const cxx::function_ref<bool()>& waitCall) noexcept
{
...
ConditionListener::NotificationVector_t ConditionListener::waitImpl(
    std::unique_lock<std::mutex>& lock, const cxx::function_ref<bool()>& waitCall) noexcept
{
  using Type_t = iox::cxx::BestFittingType_t<iox::MAX_NUMBER_OF_EVENTS_PER_LISTENER>;
  NotificationVector_t activeNotifications;
