...
namespace capro
{
ServiceDescription::ClassHash::ClassHash() noexcept : ClassHash{0U, 0U, 0U, 0U}
{
}
...
ServiceDescription::ServiceDescription() noexcept : ServiceDescription("", "", "")
{
}

ServiceDescription::ServiceDescription(const IdString_t& service, const IdString_t& instance,
    const IdString_t& event, ClassHash classHash, Interfaces interfaceSource) noexcept :
    m_serviceString{service},
    m_instanceString{instance}, m_eventString{event}, m_classHash(classHash),
...
ServiceDescription::operator cxx::Serialization() const noexcept
{
  ...
  std::underlying_type<Interfaces>::type interface = static_cast<std::underlying_type<Interfaces>::type>(
      m_interfaceSource);
  return cxx::Serialization::create(m_serviceString, m_instanceString, m_eventString, m_classHash[0U],
      m_classHash[1U], m_classHash[2U], m_classHash[3U], scope, interface);
}

cxx::expected<ServiceDescription, cxx::Serialization::Error> ServiceDescription::deserialize(
    const cxx::Serialization& serialized) noexcept
{
  ...
  auto deserializationSuccessful = serialized.extract(deserializedObject.m_serviceString,
      deserializedObject.m_instanceString, deserializedObject.m_eventString,
      deserializedObject.m_classHash[0U], deserializedObject.m_classHash[1U],
      deserializedObject.m_classHash[2U], deserializedObject.m_classHash[3U], scope, interfaceSource);
