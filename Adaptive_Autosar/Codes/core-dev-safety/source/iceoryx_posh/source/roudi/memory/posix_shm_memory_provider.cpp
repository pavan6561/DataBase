...
PosixShmMemoryProvider::PosixShmMemoryProvider(
    const ShmName_t& shmName, const posix::AccessMode accessMode, const posix::OpenMode openMode) noexcept :
    m_shmName(shmName),
    m_accessMode(accessMode), m_openMode(openMode)
{
}
...
cxx::expected<void*, MemoryProviderError> PosixShmMemoryProvider::createMemory(
