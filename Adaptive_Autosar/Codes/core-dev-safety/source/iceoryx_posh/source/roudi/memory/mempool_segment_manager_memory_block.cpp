...
MemPoolSegmentManagerMemoryBlock::MemPoolSegmentManagerMemoryBlock(
    const mepoo::SegmentConfig& segmentConfig) noexcept :
...
{
  return cxx::align(
             static_cast<uint64_t>(sizeof(mepoo::SegmentManager<>)), mepoo::MemPool::CHUNK_MEMORY_ALIGNMENT)
...
uint64_t MemPoolSegmentManagerMemoryBlock::alignment() const noexcept
{
  return algorithm::max(
      static_cast<uint64_t>(alignof(mepoo::SegmentManager<>)), mepoo::MemPool::CHUNK_MEMORY_ALIGNMENT);
}
