...
PortManager::PortManager(RouDiMemoryInterface* roudiMemoryInterface) noexcept
{
  ...
  m_serviceRegistryPublisherPortData = acquireInternalPublisherPortDataWithoutDiscovery(
      ...
      registryPortOptions, introspectionMemoryManager);
  ...
  auto portGeneric
      = acquireInternalPublisherPortData(IntrospectionPortService, options, introspectionMemoryManager);

  auto portThroughput = acquireInternalPublisherPortData(
      IntrospectionPortThroughputService, options, introspectionMemoryManager);
...
void PortManager::doDiscoveryForPublisherPort(PublisherPortRouDiType& publisherPort) noexcept
{
  publisherPort.tryGetCaProMessage().and_then([this, &publisherPort](auto caproMessage) {
    ...
    else
    {
      ...
      errorHandler(
          PoshError::PORT_MANAGER__HANDLE_PUBLISHER_PORTS_INVALID_CAPRO_MESSAGE, iox::ErrorLevel::MODERATE);
      return;
    }
  ...
  subscriberPort.tryGetCaProMessage().and_then([this, &subscriberPort](auto caproMessage) {
        ...
        || (capro::CaproMessageType::UNSUB == caproMessage.m_type))
    {
      ...
      if (!this->sendToAllMatchingPublisherPorts(caproMessage, subscriberPort))
      {
        ...
        capro::CaproMessage nackMessage(
            capro::CaproMessageType::NACK, subscriberPort.getCaProServiceDescription());
    ...
    else
    {
      LogWarn() << "CaPro protocol error for subscriber from runtime '" << subscriberPort.getRuntimeName()
                ...
                << "'! Cannot handle CaProMessageType '" << caproMessage.m_type << "'";
      errorHandler(
          PoshError::PORT_MANAGER__HANDLE_SUBSCRIBER_PORTS_INVALID_CAPRO_MESSAGE, iox::ErrorLevel::MODERATE);
      return;
    }
  });
}
  ...
  clientPort.tryGetCaProMessage().and_then([this, &clientPort](auto caproMessage) {
        ...
        || (capro::CaproMessageType::DISCONNECT == caproMessage.m_type))
    {
      ...
      if (!this->sendToAllMatchingServerPorts(caproMessage, clientPort))
      {
        LogDebug() << "capro::CONNECT/DISCONNECT, no matching server for client from runtime '"
                   ...
                   << caproMessage.m_serviceDescription << "'!";
        capro::CaproMessage nackMessage(
            capro::CaproMessageType::NACK, clientPort.getCaProServiceDescription());
    ...
    else
    {
      LogWarn() << "CaPro protocol error for client from runtime '" << clientPort.getRuntimeName()
                ...
                << "'! Cannot handle CaProMessageType '" << caproMessage.m_type << "'";
      errorHandler(
          PoshError::PORT_MANAGER__HANDLE_CLIENT_PORTS_INVALID_CAPRO_MESSAGE, iox::ErrorLevel::MODERATE);
      return;
    }
  });
}
  ...
  serverPort.tryGetCaProMessage().and_then([this, &serverPort](auto caproMessage) {
    ...
    else
    {
      LogWarn() << "CaPro protocol error for server from runtime '" << serverPort.getRuntimeName()
                ...
                << "'! Cannot handle CaProMessageType '" << caproMessage.m_type << "'";
      errorHandler(
          PoshError::PORT_MANAGER__HANDLE_SERVER_PORTS_INVALID_CAPRO_MESSAGE, iox::ErrorLevel::MODERATE);
      return;
    }
...
bool PortManager::isCompatiblePubSub(
    const PublisherPortRouDiType& publisher, const SubscriberPortType& subscriber) const noexcept
{
  ...
  const bool historyRequestIsCompatible
      = !subOpts.requiresPublisherHistorySupport || pubOpts.historyCapacity > 0;

  return blockingPoliciesAreCompatible && historyRequestIsCompatible;
}

bool PortManager::sendToAllMatchingPublisherPorts(
    const capro::CaproMessage& message, SubscriberPortType& subscriberSource) noexcept
{
...
void PortManager::sendToAllMatchingSubscriberPorts(
    const capro::CaproMessage& message, PublisherPortRouDiType& publisherSource) noexcept
{
  for (auto subscriberPortData : m_portPool->getSubscriberPortDataList())
  {
    SubscriberPortType subscriberPort(subscriberPortData);
...
bool PortManager::isCompatibleClientServer(
    const popo::ServerPortRouDi& server, const popo::ClientPortRouDi& client) const noexcept
{
...
void PortManager::sendToAllMatchingClientPorts(
    const capro::CaproMessage& message, popo::ServerPortRouDi& serverSource) noexcept
{
  for (auto clientPortData : m_portPool->getClientPortDataList())
  {
    popo::ClientPortRouDi clientPort(*clientPortData);
...
bool PortManager::sendToAllMatchingServerPorts(
    const capro::CaproMessage& message, popo::ClientPortRouDi& clientSource) noexcept
{
...
cxx::expected<PublisherPortRouDiType::MemberType_t*, PortPoolError> PortManager::acquirePublisherPortData(
    const capro::ServiceDescription& service, const popo::PublisherOptions& publisherOptions,
    const RuntimeName_t& runtimeName, mepoo::MemoryManager* const payloadDataSegmentMemoryManager,
...
cxx::expected<PublisherPortRouDiType::MemberType_t*, PortPoolError>
    PortManager::acquirePublisherPortDataWithoutDiscovery(const capro::ServiceDescription& service,
        const popo::PublisherOptions& publisherOptions, const RuntimeName_t& runtimeName,
        ...
        const PortConfigInfo& portConfigInfo) noexcept
{
  ...
  {
    errorHandler(
        PoshError::POSH__PORT_MANAGER_INTERNAL_SERVICE_DESCRIPTION_IS_FORBIDDEN, ErrorLevel::MODERATE);
...
PublisherPortRouDiType::MemberType_t* PortManager::acquireInternalPublisherPortData(
    const capro::ServiceDescription& service, const popo::PublisherOptions& publisherOptions,
...
cxx::expected<SubscriberPortType::MemberType_t*, PortPoolError> PortManager::acquireSubscriberPortData(
    const capro::ServiceDescription& service, const popo::SubscriberOptions& subscriberOptions,
    const RuntimeName_t& runtimeName, const PortConfigInfo& portConfigInfo) noexcept
{
...
cxx::expected<popo::ClientPortData*, PortPoolError> PortManager::acquireClientPortData(
    const capro::ServiceDescription& service, const popo::ClientOptions& clientOptions,
    const RuntimeName_t& runtimeName, mepoo::MemoryManager* const payloadDataSegmentMemoryManager,
    const PortConfigInfo& portConfigInfo) noexcept
{
  // we can create a new port
  return m_portPool
      ->addClientPort(
          service, payloadDataSegmentMemoryManager, runtimeName, clientOptions, portConfigInfo.memoryInfo)
...
cxx::expected<popo::ServerPortData*, PortPoolError> PortManager::acquireServerPortData(
    const capro::ServiceDescription& service, const popo::ServerOptions& serverOptions,
    const RuntimeName_t& runtimeName, mepoo::MemoryManager* const payloadDataSegmentMemoryManager,
    const PortConfigInfo& portConfigInfo) noexcept
{
  ...
  return m_portPool
      ->addServerPort(
          service, payloadDataSegmentMemoryManager, runtimeName, serverOptions, portConfigInfo.memoryInfo)
...
popo::InterfacePortData* PortManager::acquireInterfacePortData(
    capro::Interfaces interface, const RuntimeName_t& runtimeName, const NodeName_t& /*node*/) noexcept
{
...
void PortManager::publishServiceRegistry() const noexcept
{
  ...
  publisher
      .tryAllocateChunk(sizeof(ServiceRegistry), alignof(ServiceRegistry), CHUNK_NO_USER_HEADER_SIZE,
...
cxx::expected<runtime::NodeData*, PortPoolError> PortManager::acquireNodeData(
