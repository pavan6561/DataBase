...
namespace roudi
{
RouDi::RouDi(RouDiMemoryInterface& roudiMemoryInterface, PortManager& portManager,
    ...
    m_runMonitoringAndDiscoveryThread(true), m_runHandleRuntimeMessageThread(true),
    m_roudiMemoryInterface(&roudiMemoryInterface), m_portManager(&portManager),
    m_prcMgr(concurrent::ForwardArgsToCTor, *m_roudiMemoryInterface, portManager,
  ...
  if (m_killProcessesInDestructor)
  {
    ...
    while (m_prcMgr->isAnyRegisteredProcessStillRunning() && !finalKillTimer.hasExpired())
    {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(PROCESS_TERMINATED_CHECK_INTERVAL.toMilliseconds()));
    }
...
void RouDi::processRuntimeMessages() noexcept
{
  ...
  while (m_runHandleRuntimeMessageThread)
  {
    ...
    {
      auto cmd = runtime::stringToIpcMessageType<iox::runtime::IpcMessageType>(
          message.getElementAtIndex(0).c_str());
...
version::VersionInfo RouDi::parseRegisterMessage(
    const runtime::IpcMessage& message, uint32_t& pid, uid_t& userId, int64_t& transmissionTimestamp) noexcept
{
...
void RouDi::processMessage(const runtime::IpcMessage& message, const iox::runtime::IpcMessageType& cmd,
    const RuntimeName_t& runtimeName) noexcept
{
  ...
  case runtime::IpcMessageType::REG:
  {
    ...
    else
    {
      ...
      registerProcess(runtimeName, pid, iox::posix::PosixUser{userId}, transmissionTimestamp,
          getUniqueSessionIdForProcess(), versionInfo);
    }
    break;
  }
  case runtime::IpcMessageType::CREATE_PUBLISHER:
  {
    ...
    {
      LogError() << "Wrong number of parameters for \"IpcMessageType::CREATE_PUBLISHER\" from \""
  ...
  case runtime::IpcMessageType::CREATE_SUBSCRIBER:
  {
    if (message.getNumberOfElements() != 5)
    {
      LogError() << "Wrong number of parameters for \"IpcMessageType::CREATE_SUBSCRIBER\" from \""
                 << runtimeName << "\"received!";
    }
    else
    {
  ...
  case runtime::IpcMessageType::CREATE_CLIENT:
  {
    ...
    else
    {
      ...
      {
        LogError() << "Deserialization of 'ClientOptions' failed when '"
                   << message.getElementAtIndex(3).c_str() << "' was provided\n";
        break;
      }
  ...
  case runtime::IpcMessageType::CREATE_SERVER:
  {
    ...
    else
    {
      ...
      {
        LogError() << "Deserialization of 'ServerOptions' failed when '"
                   << message.getElementAtIndex(3).c_str() << "' was provided\n";
        break;
      }
  ...
  case runtime::IpcMessageType::CREATE_INTERFACE:
  {
    ...
    {
      LogError() << "Wrong number of parameters for \"IpcMessageType::CREATE_INTERFACE\" from \""
                 << runtimeName << "\"received!";
    }
    else
    {
      capro::Interfaces interface = StringToCaProInterface(
          capro::IdString_t(cxx::TruncateToCapacity, message.getElementAtIndex(2)));
...
void RouDi::registerProcess(const RuntimeName_t& name, const uint32_t pid, const posix::PosixUser user,
    const int64_t transmissionTimestamp, const uint64_t sessionId,
    const version::VersionInfo& versionInfo) noexcept
{
  bool monitorProcess = (m_monitoringMode == roudi::MonitoringMode::ON);
  IOX_DISCARD_RESULT(m_prcMgr->registerProcess(
      name, pid, user, monitorProcess, transmissionTimestamp, sessionId, versionInfo));
}
