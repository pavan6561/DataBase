...
ProcessManager::ProcessManager(RouDiMemoryInterface& roudiMemoryInterface, PortManager& portManager,
    ...
    m_portManager(portManager), m_compatibilityCheckLevel(compatibilityCheckLevel)
{
...
bool ProcessManager::requestShutdownOfProcess(Process& process, ShutdownPolicy shutdownPolicy) noexcept
{
  ...
  return !posix::posixCall(kill)(
      static_cast<pid_t>(process.getPid()), (shutdownPolicy == ShutdownPolicy::SIG_KILL) ? SIGKILL : SIGTERM)
              .failureReturnValue(ERROR_CODE)
              .evaluate()
              .or_else([&](auto& r) {
                this->evaluateKillError(
                    process, r.errnum, r.getHumanReadableErrnum().c_str(), shutdownPolicy);
              })
              .has_error();
}
...
bool ProcessManager::isProcessAlive(const Process& process) noexcept
{
  ...
  auto checkCommand = posix::posixCall(kill)(static_cast<pid_t>(process.getPid()), SIGTERM)
                          ...
                          .evaluate()
                          .or_else([&](auto& r) {
                            this->evaluateKillError(process, r.errnum, r.getHumanReadableErrnum().c_str(),
                                ShutdownPolicy::SIG_TERM);
                          });

  return !(checkCommand && checkCommand->errnum == ESRCH);
}

void ProcessManager::evaluateKillError(const Process& process, const int32_t& errnum, const char* errorString,
...
bool ProcessManager::registerProcess(const RuntimeName_t& name, const uint32_t pid,
    const posix::PosixUser user, const bool isMonitored, const int64_t transmissionTimestamp,
    const uint64_t sessionId, const version::VersionInfo& versionInfo) noexcept
{
...
bool ProcessManager::addProcess(const RuntimeName_t& name, const uint32_t pid, const posix::PosixUser& user,
    const bool isMonitored, const int64_t transmissionTimestamp, const uint64_t sessionId,
    const version::VersionInfo& versionInfo) noexcept
{
  ...
  m_processIntrospection->addProcess(
      static_cast<int>(pid), RuntimeName_t(cxx::TruncateToCapacity, name.c_str()));
...
bool ProcessManager::searchForProcessAndRemoveIt(
    const RuntimeName_t& name, const TerminationFeedback feedback) noexcept
{
...
bool ProcessManager::removeProcessAndDeleteRespectiveSharedMemoryObjects(
    ProcessList_t::iterator& processIter, const TerminationFeedback feedback) noexcept
{
...
void ProcessManager::addInterfaceForProcess(
    const RuntimeName_t& name, capro::Interfaces interface, const NodeName_t& node) noexcept
{
  findProcess(name)
      .and_then([&](auto& process) {
  ...
  findProcess(name)
      .and_then([&](auto& process) {
        ...
        else
        {
          ...
          sendBuffer << runtime::IpcMessageTypeToString(runtime::IpcMessageType::ERROR);
          sendBuffer << runtime::IpcMessageErrorTypeToString(
              runtime::IpcMessageErrorType::SUBSCRIBER_LIST_FULL);
          process->sendViaIpcChannel(sendBuffer);
      ...
      .or_else([&]() {
        LogWarn() << "Unknown application '" << name
    ...
    const PortConfigInfo& portConfigInfo) noexcept
{
  findProcess(name)
      ...
      .or_else([&]() {
        LogWarn() << "Unknown application '" << name
                  << "' requested a PublisherPort with service description '" << service << "'";
      });
}

void ProcessManager::addClientForProcess(const RuntimeName_t& name, const capro::ServiceDescription& service,
    const popo::ClientOptions& clientOptions, const PortConfigInfo& portConfigInfo) noexcept
{
  findProcess(name)
...
void ProcessManager::addServerForProcess(const RuntimeName_t& name, const capro::ServiceDescription& service,
    const popo::ServerOptions& serverOptions, const PortConfigInfo& portConfigInfo) noexcept
{
  findProcess(name)
...
popo::PublisherPortData* ProcessManager::addIntrospectionPublisherPort(
