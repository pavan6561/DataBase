    ...
    iox::config::TomlGatewayConfigParser::parse(const roudi::ConfigFilePathString_t& path) noexcept
{
  ...
  catch (const std::exception& parserException)
  {
    ...
    LogWarn() << iox::cxx::convertEnumToString(
                     iox::config::TOML_GATEWAY_CONFIG_FILE_PARSE_ERROR_STRINGS, parserError)
...
iox::cxx::expected<iox::config::TomlGatewayConfigParseError> iox::config::TomlGatewayConfigParser::validate(
    const cpptoml::table& parsedToml) noexcept
{
  ...
  if (!serviceArray)
  {
    return iox::cxx::error<TomlGatewayConfigParseError>(
        TomlGatewayConfigParseError::INCOMPLETE_CONFIGURATION);
  }
