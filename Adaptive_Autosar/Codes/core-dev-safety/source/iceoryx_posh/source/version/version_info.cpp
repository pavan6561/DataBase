...
VersionInfo::VersionInfo(const uint16_t versionMajor, const uint16_t versionMinor,
    const uint16_t versionPatch, const uint16_t versionTweak, const BuildDateString_t& buildDateString,
    const CommitIdString_t& commitIdString) noexcept :
    m_versionMajor(versionMajor),
    m_versionMinor(versionMinor), m_versionPatch(versionPatch), m_versionTweak(versionTweak),
    m_buildDateString(buildDateString), m_commitIdString(commitIdString)
{
}
...
VersionInfo::VersionInfo(const cxx::Serialization& serial) noexcept
{
  ...
  m_valid = serial.extract(m_versionMajor, m_versionMinor, m_versionPatch, m_versionTweak,
      tmp_m_buildDateString, tmp_commitIdString);
...
VersionInfo::operator cxx::Serialization() const noexcept
{
  ...
  return cxx::Serialization::create(m_versionMajor, m_versionMinor, m_versionPatch, m_versionTweak,
      tmp_m_buildDateString, tmp_commitIdString);
}

bool VersionInfo::operator==(const VersionInfo& rhs) const noexcept
{
  return (m_valid == rhs.m_valid) && (m_versionMajor == rhs.m_versionMajor)
      && (m_versionMinor == rhs.m_versionMinor) && (m_versionPatch == rhs.m_versionPatch)
      && (m_versionTweak == rhs.m_versionTweak) && (m_buildDateString == rhs.m_buildDateString)
      && (m_commitIdString == rhs.m_commitIdString);
}
...
VersionInfo VersionInfo::getCurrentVersion() noexcept
{
  ...
  return VersionInfo(static_cast<uint16_t>(ICEORYX_VERSION_MAJOR),
      static_cast<uint16_t>(ICEORYX_VERSION_MINOR), static_cast<uint16_t>(ICEORYX_VERSION_PATCH),
      static_cast<uint16_t>(ICEORYX_VERSION_TWEAK), buildDateStringCxx, shortCommitIdString);
}
