...
static HANDLE acquireMutexHandle(pthread_mutex_t* mutex)
{
  ...
  newHandle
      = Win32Call(OpenMutexA, MUTEX_ALL_ACCESS, false, generateMutexName(mutex->uniqueId).c_str()).value;
  if (newHandle == nullptr)
  {
    fprintf(stderr, "interprocess mutex %s is corrupted - segmentation fault immenent\n",
