...
static HANDLE acquireSemaphoreHandle(iox_sem_t* sem)
{
  ...
  newHandle
      = Win32Call(OpenSemaphoreA, SEMAPHORE_ALL_ACCESS, false, generateSemaphoreName(sem->uniqueId).c_str())
            .value;
  if (newHandle == nullptr)
  {
    fprintf(stderr, "interprocess semaphore %s is corrupted - segmentation fault immenent\n",
...
int iox_sem_timedwait(iox_sem_t* sem, const struct timespec* abs_timeout)
{
  ...
  long milliseconds = (epochCurrentTimeDiffInNanoSeconds
                          + (abs_timeout->tv_nsec - tv.tv_usec * NANO_SECONDS_PER_MICRO_SECOND)
                          + HALF_MILLI_SECOND_ROUNDING_CORRECTION_IN_NS)
      / NANO_SECONDS_PER_MILLI_SECOND;

  auto state
      = Win32Call(WaitForSingleObject, acquireSemaphoreHandle(sem), (milliseconds == 0) ? 1 : milliseconds)
            .value;
...
static HANDLE sem_create_win32_semaphore(LONG value, LPCSTR name)
{
  ...
  Win32Call(ConvertStringSecurityDescriptorToSecurityDescriptor, reinterpret_cast<LPCSTR>(permissions),
  ...
  HANDLE returnValue
      = Win32Call(CreateSemaphoreA, &securityAttribute, value, MAX_SEMAPHORE_VALUE, name).value;
  return returnValue;
}
