...
UniqueSystemId::UniqueSystemId() noexcept :
    m_processId{GetCurrentProcessId()}, m_timestamp{static_cast<uint64_t>(
...
{
  return m_processId == rhs.m_processId && m_timestamp == rhs.m_timestamp
...
{
  return std::to_string(m_processId) + "_" + std::to_string(m_timestamp) + "_"
