...
static std::chrono::nanoseconds getNanoSeconds(const timespec& value)
{
  ...
  return std::chrono::nanoseconds(
      static_cast<uint64_t>(value.tv_sec) * NANOSECONDS + static_cast<uint64_t>(value.tv_nsec));
}
...
static bool waitForExecution(timer_t timerid)
{
  ...
  if (timerid->parameter.isTimerRunning)
  {
    ...
    timerid->parameter.wakeup.wait_until(ulock, timePoint_t(getNanoSeconds(waitUntil)),
        [timerid] { return !timerid->parameter.isTimerRunning || !timerid->parameter.keepRunning; });
  }
  else
  {
...
static void setTimeParameters(
    timer_t timerid, const itimerspec& timeParameters, const bool runOnce, const bool isTimerRunning)
{
...
int gettimeofday(struct timeval* tp, struct timezone* tzp)
{
  ...
  uint64_t time = static_cast<uint64_t>(fileTime.dwLowDateTime)
