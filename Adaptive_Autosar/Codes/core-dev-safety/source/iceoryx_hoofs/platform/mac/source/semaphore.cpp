...
int iox_sem_timedwait(iox_sem_t* sem, const struct timespec* abs_timeout)
{
  ...
  int64_t timeoutInNanoSeconds = std::max(0ll,
      (abs_timeout->tv_sec - tv.tv_sec) * NANOSECONDS_PER_SECOND + abs_timeout->tv_nsec
          - tv.tv_usec * NANOSECONDS_PER_MICROSECOND);
  ...
  else
  {
    ...
    if (sem->m_value.load(std::memory_order_relaxed) == 0)
    {
      int result = pthread_cond_timedwait(
          &sem->m_handle.condition.variable, &sem->m_handle.condition.mtx, abs_timeout);
      if (result != 0)
      {
        if (result == ETIMEDOUT)
        {
