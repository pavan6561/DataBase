...
namespace posix
{
cxx::expected<SemaphoreError> UnnamedSemaphoreBuilder::create(
    cxx::optional<UnnamedSemaphore>& uninitializedSemaphore) const noexcept
{
  ...
  auto result = posixCall(iox_sem_init)(&uninitializedSemaphore.value().m_handle,
      (m_isInterProcessCapable) ? 1 : 0, static_cast<unsigned int>(m_initialValue))
