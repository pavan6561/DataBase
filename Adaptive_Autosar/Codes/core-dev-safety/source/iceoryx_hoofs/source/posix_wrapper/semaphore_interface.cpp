...
template <typename SemaphoreChild>
cxx::expected<SemaphoreWaitState, SemaphoreError> SemaphoreInterface<SemaphoreChild>::timedWait(
    const units::Duration& timeout) noexcept
{
  ...
  return cxx::success<SemaphoreWaitState>(
      (result.value().errnum == ETIMEDOUT) ? SemaphoreWaitState::TIMEOUT : SemaphoreWaitState::NO_TIMEOUT);
}
...
{
  auto result
      = posixCall(iox_sem_trywait)(getHandle()).failureReturnValue(-1).ignoreErrnos(EAGAIN).evaluate();
