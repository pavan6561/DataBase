...
NamedPipe::NamedPipe(const IpcChannelName_t& name, const IpcChannelSide channelSide, const size_t maxMsgSize,
    const uint64_t maxMsgNumber) noexcept
{
  ...
  m_data
      = static_cast<NamedPipeData*>(m_sharedMemory->allocate(sizeof(NamedPipeData), alignof(NamedPipeData)));
...
cxx::expected<IpcChannelError> NamedPipe::timedSend(
    const std::string& message, const units::Duration& timeout) const noexcept
{
  if (!m_isInitialized)
  {
    return cxx::error<IpcChannelError>(IpcChannelError::NOT_INITIALIZED);
...
cxx::expected<std::string, IpcChannelError> NamedPipe::timedReceive(const units::Duration& timeout) const
    noexcept
{
  if (!m_isInitialized)
  {
...
NamedPipe::NamedPipeData::NamedPipeData(
    bool& isInitialized, IpcChannelError& error, const uint64_t maxMsgNumber) noexcept
{
  ...
  Semaphore::placementCreate(&semaphores[SEND_SEMAPHORE], CreateUnnamedSharedMemorySemaphore,
