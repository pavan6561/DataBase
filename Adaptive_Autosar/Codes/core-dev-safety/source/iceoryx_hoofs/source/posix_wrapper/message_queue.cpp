...
MessageQueue::MessageQueue(const IpcChannelName_t& name, const IpcChannelSide channelSide,
...
cxx::expected<bool, IpcChannelError> MessageQueue::unlinkIfExists(const IpcChannelName_t& name) noexcept
{
  ...
  if (sanitizeIpcChannelName(name)
          .and_then([&](IpcChannelName_t& name) { l_name = std::move(name); })
          .has_error())
  {
  ...
  auto mqCall
      = posixCall(mq_unlink)(l_name.c_str()).failureReturnValue(ERROR_CODE).ignoreErrnos(ENOENT).evaluate();
...
cxx::expected<IpcChannelError> MessageQueue::send(const std::string& msg) const noexcept
{
  ...
  auto mqCall = posixCall(mq_send)(m_mqDescriptor, msg.c_str(), messageSize, 1U)
                    .failureReturnValue(ERROR_CODE)
                    .evaluate();
...
cxx::expected<mqd_t, IpcChannelError> MessageQueue::open(
    const IpcChannelName_t& name, const IpcChannelSide channelSide) noexcept
{
  IpcChannelName_t l_name;
  if (sanitizeIpcChannelName(name)
          .and_then([&](IpcChannelName_t& name) { l_name = std::move(name); })
    ...
    noexcept
{
  ...
  auto mqCall
      = posixCall(mq_timedreceive)(m_mqDescriptor, message, MAX_MESSAGE_SIZE, nullptr, &timeOut)
            .failureReturnValue(ERROR_CODE)
...
cxx::expected<IpcChannelError> MessageQueue::timedSend(
    const std::string& msg, const units::Duration& timeout) const noexcept
{
  ...
  auto mqCall
      = posixCall(mq_timedsend)(m_mqDescriptor, msg.c_str(), messageSize, 1U, &timeOut)
            .failureReturnValue(ERROR_CODE)
            // don't use the suppressErrorMessagesForErrnos method since QNX used EINTR instead of ETIMEDOUT
            .ignoreErrnos(TIMEOUT_ERRNO)
            .evaluate();
...
cxx::error<IpcChannelError> MessageQueue::createErrorFromErrnum(
    const IpcChannelName_t& name, const int32_t errnum) noexcept
{
  switch (errnum)
  {
  case EACCES:
...
cxx::expected<IpcChannelName_t, IpcChannelError> MessageQueue::sanitizeIpcChannelName(
