...
bool AccessController::writePermissionsToFile(const int32_t f_fileDescriptor) const noexcept
{
  ...
  auto aclSetFdCall
      = posixCall(acl_set_fd)(f_fileDescriptor, workingACL.get()).successReturnValue(0).evaluate();
...
bool AccessController::addPermissionEntry(
    const Category f_category, const Permission f_permission, const string_t& f_name) noexcept
{
  switch (f_category)
  {
...
bool AccessController::addPermissionEntry(
    const Category f_category, const Permission f_permission, const uint32_t f_id) noexcept
{
...
bool AccessController::createACLEntry(const acl_t f_ACL, const PermissionEntry& f_entry) noexcept
{
  ...
  auto aclGetPermsetCall
      = posixCall(acl_get_permset)(newEntry, &entryPermissionSet).successReturnValue(0).evaluate();
