...
PosixRights::PosixRights(bool f_read, bool f_write, bool f_execute) noexcept :
    m_read(f_read), m_write(f_write), m_execute(f_execute)
{
}

PosixGroup::PosixGroup(gid_t f_id) noexcept : m_id(f_id), m_doesExist(getGroupName(f_id).has_value())
{
}
...
PosixUser::PosixUser(uid_t f_id) noexcept : m_id(f_id), m_doesExist(getUserName(f_id).has_value())
{
}
