...
Timer::OsTimer::OsTimer(const units::Duration timeToWait, const std::function<void()>& callback) noexcept :
    m_timeToWait(timeToWait), m_callback(callback)
{
  ...
  asyncCallNotification.sigev_value.sival_ptr
      = nullptr; // initialize all bits of the sigval union for mem check
  asyncCallNotification.sigev_value.sival_int
      = Timer::OsTimerCallbackHandle::indexAndDescriptorToSigval(
          m_callbackHandleIndex, callbackHandleDescriptor)
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access) system struct
            .sival_int;
...
cxx::expected<TimerError> Timer::OsTimer::start(
    const RunMode runMode, const CatchUpPolicy catchUpPolicy) noexcept
{
...
cxx::expected<TimerError> Timer::OsTimer::restart(
    const units::Duration timeToWait, const RunMode runMode, const CatchUpPolicy catchUpPolicy) noexcept
{
...
cxx::expected<units::Duration, TimerError> Timer::OsTimer::timeUntilExpiration() noexcept
{
  ...
  {
    // Timer is disarmed
    OsTimer::s_callbackHandlePool[m_callbackHandleIndex].m_isTimerActive.store(
...
Timer::Timer(const units::Duration timeToWait) noexcept :
    m_timeToWait(timeToWait), m_creationTime(now().value())
{
...
cxx::expected<TimerError> Timer::restart(
    const units::Duration timeToWait, const RunMode runMode, const CatchUpPolicy catchUpPolicy) noexcept
{
