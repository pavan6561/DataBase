...
cxx::expected<SharedMemory, SharedMemoryError> SharedMemoryBuilder::create() noexcept
{
  ...
  mode_t umaskSaved = umask(0U);
  {
    ...
    auto result = posixCall(iox_shm_open)(nameWithLeadingSlash.c_str(),
        getOflagsFor(
            m_accessMode, (m_openMode == OpenMode::OPEN_OR_CREATE) ? OpenMode::EXCLUSIVE_CREATE : m_openMode),
    ...
    if (result.has_error())
    {
      ...
      if (m_openMode == OpenMode::OPEN_OR_CREATE && result.get_error().errnum == EEXIST)
      {
        result = posixCall(iox_shm_open)(nameWithLeadingSlash.c_str(),
            getOflagsFor(m_accessMode, OpenMode::OPEN_EXISTING), static_cast<mode_t>(m_filePermissions))
                     .failureReturnValue(SharedMemory::INVALID_HANDLE)
                     .evaluate();
        if (!result.has_error())
        {
  ...
  const bool hasOwnership = (m_openMode == OpenMode::EXCLUSIVE_CREATE
      || m_openMode == OpenMode::PURGE_AND_CREATE || m_openMode == OpenMode::OPEN_OR_CREATE);
  if (hasOwnership)
  {
...
bool SharedMemory::close() noexcept
{
  ...
  {
    auto call
        = posixCall(iox_close)(m_handle).failureReturnValue(INVALID_HANDLE).evaluate().or_else([](auto& r) {
