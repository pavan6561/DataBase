...
cxx::expected<MemoryMap, MemoryMapError> MemoryMapBuilder::create() noexcept
{
  ...
  auto result = posixCall(mmap)(const_cast<void*>(m_baseAddressHint), m_length, l_memoryProtection,
      static_cast<int32_t>(m_flags), m_fileDescriptor, m_offset)
  ...
  std::cerr << "Unable to map memory with the following properties [ baseAddressHint = " << std::hex
            << m_baseAddressHint << ", length = " << std::dec << m_length
            ...
            << ", offset = " << std::hex << m_offset << std::dec << " ]" << std::endl;
  std::cerr.setf(flags);
...
MemoryMapError MemoryMap::errnoToEnum(const int32_t errnum) noexcept
{
  ...
  case ETXTBSY:
    std::cerr << "The memory region was set up with MAP_DENYWRITE but write access was requested."
              << std::endl;
