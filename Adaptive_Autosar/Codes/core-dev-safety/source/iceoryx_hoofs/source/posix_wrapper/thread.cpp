...
{
  posixCall(iox_pthread_setname_np)(thread, name.c_str())
...
cxx::expected<ThreadError> ThreadBuilder::create(
    cxx::optional<Thread>& uninitializedThread, const Thread::callable_t& callable) noexcept
{
  uninitializedThread.emplace(m_name, callable);

  const pthread_attr_t* threadAttributes = nullptr;

  auto createResult = posixCall(iox_pthread_create)(&uninitializedThread->m_threadHandle, threadAttributes,
      Thread::startRoutine, &uninitializedThread.value())
                          .successReturnValue(0)
                          .evaluate();
...
Thread::Thread(const ThreadName_t& name, const callable_t& callable) noexcept :
    m_threadHandle{}, m_callable{callable}, m_isThreadConstructed{false}, m_threadName{name}
{
}
