...
FileLock::FileLock(const FileName_t& name) noexcept :
    m_name(name), m_fileLockPath(platform::IOX_LOCK_FILE_PATH_PREFIX + m_name + LOCK_FILE_SUFFIX)
{
  initializeFileLock()
      .and_then([this]() { this->m_isInitialized = true; })
      .or_else([this](FileLockError& error) {
...
cxx::expected<FileLockError> FileLock::closeFileDescriptor() noexcept
{
  if (isInitialized() && (m_fd != INVALID_FD))
  {
    ...
    posixCall(iox_close)(m_fd).failureReturnValue(ERROR_CODE).evaluate().or_else([&](auto& result) {
      cleanupFailed = true;
      IOX_DISCARD_RESULT(this->convertErrnoToFileLockError(result.errnum));
      std::cerr << "Unable to close the file handle to the file lock \"" << m_fileLockPath << "\""
                << std::endl;
    });

    posixCall(remove)(m_fileLockPath.c_str())
        .failureReturnValue(ERROR_CODE)
        .evaluate()
        .or_else([&](auto& result) {
          cleanupFailed = true;
