...
cxx::expected<SemaphoreWaitState, SemaphoreError> Semaphore::timedWait(
    const units::Duration abs_timeout) noexcept
{
  const struct timespec timeout = abs_timeout.timespec(units::TimeSpecReference::Epoch);
  auto call = posixCall(iox_sem_timedwait)(getHandle(), &timeout)
...
Semaphore::Semaphore(OpenNamedSemaphore_t, const char* name, const int oflag) noexcept : m_isCreated(false)
{
...
Semaphore::Semaphore(
    CreateNamedSemaphore_t, const char* name, const mode_t mode, const unsigned int value) noexcept :
    m_isCreated(true)
{
  if (!m_name.unsafe_assign(name))
  {
