...
// -----------------------------------------------------------------------------------------------------------
// File Name   : crypto_object_proxy_impl.h
//
// Purpose     : Crypto object proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_CRYPTO_OBJECT_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_CRYPTO_OBJECT_PROXY_IMPL_H_

#include <ara/log/logging.h>

#include "ara/crypto/cryp/cryobj/crypto_object.h"
#include "crypto_primitive_id_proxy_impl.h"
#include "cryptoobject_proxy.h"
...
class CryptoObjectProxyImpl : public ara::crypto::cryp::CryptoObject
{
 public:
  /// @brief Constructor of CryptoObjectProxyImpl class.
  /// @param[in] serviceInstanceID of Crypto service instance id.
  CryptoObjectProxyImpl(const CryptoServiceInstanceId serviceInstanceID) noexcept;

  /// @brief Destructor used to destruct the object of CryptoObjectProxyImpl.
  virtual ~CryptoObjectProxyImpl() noexcept = default;

  /// @brief Method for Get the crypto service instance id.
  /// @returns Crypto service instance id of CryptoObjectProxyImpl.
  CryptoServiceInstanceId GetCryptoObjectServiceInstanceId() const
  {
    return this->cryptoObjServiceInstanceID_;
  }
 ...
 private:
  /// @brief Method to initializing the server-client connection for CryptoObjectProxyImpl contex.
  ...
  ara::crypto::vsomeip::VsomeipInterface<
    ara::crypto::ipccom::cryp::cryobj::cryptoobject::proxy::CryptoObjectProxy>
    vsomeipInstance_;

  /// @brief Crypto provider handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryptoprovider::proxy::CryptoProviderInterfaceProxy>
    cryptoObjCryptoProviderHandler_;

  /// @brief Symmetric key wrapper context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::symmetrickeywrapper::proxy::SymmetricKeyWrapperContextProxy>
    symmetricKeyWrapHandler_;

  /// @brief Crypto Object context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryobj::cryptoobject::proxy::CryptoObjectProxy>
    cryptoObjectHandler_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_CRYPTO_OBJECT_PROXY_IMPL_H_
