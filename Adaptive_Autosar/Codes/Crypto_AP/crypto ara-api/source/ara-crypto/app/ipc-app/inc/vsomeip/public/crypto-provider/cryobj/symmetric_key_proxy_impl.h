...
// -----------------------------------------------------------------------------------------------------------
// File Name   : symmetric_key_proxy_impl.h
//
// Purpose     : Symmetric key proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SYMMETRIC_KEY_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SYMMETRIC_KEY_PROXY_IMPL_H_
...
class SymmetricKeyProxyImpl : public ara::crypto::cryp::SymmetricKey
{
 public:
  /// @brief Constructor of SymmetricKeyProxyImpl class.
  /// @param[in] serviceInstanceID of Crypto service instance id.
  SymmetricKeyProxyImpl(const CryptoServiceInstanceId serviceInstanceID) noexcept;

  /// @brief Destructor used to destruct the object of SymmetricKeyProxyImpl.
  virtual ~SymmetricKeyProxyImpl() noexcept = default;

  /// @brief Method to Get the crypto service instance id.
  /// @returns Crypto service instance id of SymmetricKeyProxyImpl.
  CryptoServiceInstanceId GetSymmetricKeyServiceInstanceId() const
  {
    return this->symmetricKeyServiceInstanceID_;
  }
 ...
 private:
  /// @brief Method to initializing the server-client connection for SymmetricKeyProxyImpl contex.
  /// @param[in] serviceInstanceID Crypto service instance id.
  void Initialize(const CryptoServiceInstanceId serviceInstanceID);

  /// @brief Vsome-ip instance for Symmetric Key context proxy.
  ara::crypto::vsomeip::VsomeipInterface<
    ara::crypto::ipccom::cryp::cryobj::symmetrickey::proxy::SymmetricKeyProxy>
    vsomeipInstance_;

  /// @brief Crypto Object context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryobj::symmetrickey::proxy::SymmetricKeyProxy>
    symmetricKeyHandler_;

  /// @brief Used to store the crypto service instance id.
  CryptoServiceInstanceId const symmetricKeyServiceInstanceID_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SYMMETRIC_KEY_PROXY_IMPL_H_
