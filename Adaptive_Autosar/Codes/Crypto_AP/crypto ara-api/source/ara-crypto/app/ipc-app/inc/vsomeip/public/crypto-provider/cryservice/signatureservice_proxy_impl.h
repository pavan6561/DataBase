...
// -----------------------------------------------------------------------------------------------------------
// File Name : signatureservice_proxy_impl.h
//
// Purpose : Crypto object proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYSERVICE_SIGNATURESERVICE_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYSERVICE_SIGNATURESERVICE_PROXY_IMPL_H_

#include <ara/log/logging.h>

#include "ara/crypto/cryp/signature_service.h"
#include "crypto_primitive_id_proxy_impl.h"
#include "signatureservice_proxy.h"
...
class SignatureServiceProxyImpl : public ara::crypto::cryp::SignatureService
{
 public:
  /// @brief Constructor of SignatureServiceProxyImpl class.
  /// @param[in] serviceInstanceID of Crypto service instance id.
  SignatureServiceProxyImpl(const CryptoServiceInstanceId serviceInstanceID) noexcept;

  /// @brief Destructor used to destruct the object of SignatureServiceProxyImpl.
  virtual ~SignatureServiceProxyImpl() noexcept = default;
  ...
  /// @brief Get actual bit-length of a key loaded to the context. If no key was set to the context yet then
  /// 0 is returned.
 ...
 private:
  /// @brief Method to initializing the server-client connection for ExtensionServiceProxy context.
  ...
  ara::crypto::vsomeip::VsomeipInterface<
      ara::crypto::ipccom::cryp::cryservice::signatureservice::proxy::SignatureServiceProxy>
      vsomeipInstance_;

  /// @brief Crypto Object context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryservice::signatureservice::proxy::SignatureServiceProxy>
      signatureServiceHandler_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYSERVICE_SIGNATURESERVICE_PROXY_IMPL_H_
