...
// -----------------------------------------------------------------------------------------------------------
// File Name   : streamcipher_context_proxy_impl.h
//
// Purpose     : Stream cipher proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_STREAMCIPHER_CONTEXT_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_STREAMCIPHER_CONTEXT_PROXY_IMPL_H_
...
#include "streamciphercontext_proxy.h"
...
class StreamCipherContextProxyImpl final : public ara::crypto::cryp::StreamCipherCtx
{
 public:
  /// @brief Constructor for the StreamCipherContextProxyImpl class.
  /// @param[in] instanceValue CryptoServiceInstance Id for the Stream cipher.
  /// @param[in] sccCPHandler CryptoProviderInterfaceProxy Shared pointer for the Stream cipher.
  /// @param[in] cp Reference to the crypto provider instance.
  StreamCipherContextProxyImpl(CryptoServiceInstanceId const instanceValue,
    std::shared_ptr<ara::crypto::ipccom::cryp::cryptoprovider::proxy::CryptoProviderInterfaceProxy>&
      sccCPHandler,
    CryptoProvider& cp) noexcept;

  /// @brief Destructor for StreamCipherContextProxyImpl class.
  ~StreamCipherContextProxyImpl() noexcept
  {
    sccCryptoProviderHandler_->StopCryptoProviderService(
      CryptoProviderServiceType::kStreamCipherCtx, vsomeipSccInstance_.serviceInstanceID_);
  }
 ...
 private:
  /// @brief Method to initializing the server-client connection for StreamCipherContextProxy context.
  /// @param[in] serviceInstanceID Crypto service instance id.
  bool Initialize(CryptoServiceInstanceId const serviceInstanceID);

  /// @brief vsomeip instance for Stream cipher context proxy.
  ara::crypto::vsomeip::VsomeipInterface<
    ara::crypto::ipccom::cryp::streamcipher::proxy::StreamCipherContextProxy>
    vsomeipSccInstance_;

  /// @brief vsomeip instance for Crypto provider proxy.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryptoprovider::proxy::CryptoProviderInterfaceProxy>
    sccCryptoProviderHandler_;

  /// @brief shared handler for Stream cipher context proxy.
  std::shared_ptr<ara::crypto::ipccom::cryp::streamcipher::proxy::StreamCipherContextProxy> sccProxyHandler_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_STREAMCIPHER_CONTEXT_PROXY_IMPL_H_
