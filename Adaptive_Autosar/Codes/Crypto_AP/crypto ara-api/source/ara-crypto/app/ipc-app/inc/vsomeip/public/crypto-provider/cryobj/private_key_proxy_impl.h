...
// -----------------------------------------------------------------------------------------------------------
// File Name   : private_key_proxy_impl.h
//
// Purpose     : Private key proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_PRIVATE_KEY_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_PRIVATE_KEY_PROXY_IMPL_H_
...
class PrivateKeyProxyImpl : public ara::crypto::cryp::PrivateKey
{
 public:
  /// @brief Constructor of PrivateKeyProxyImpl class.
  /// @param[in] serviceInstanceID of Crypto service instance id.
  PrivateKeyProxyImpl(const CryptoServiceInstanceId serviceInstanceID) noexcept;

  /// @brief Destructor used to destruct the object of PrivateKeyProxyImpl.
  virtual ~PrivateKeyProxyImpl() noexcept = default;

  /// @brief Method for Get the crypto service instance id.
  /// @returns Crypto service instance id of PrivateKeyProxyImpl.
  CryptoServiceInstanceId GetPrivateKeyServiceInstanceId() const
  {
    return this->privateKeyServiceInstanceID_;
  }
 ...
 private:
  /// @brief Method to initializing the server-client connection for PrivateKeyProxyImpl contex.
  /// @param[in] serviceInstanceID Crypto service instance id.
  void Initialize(const CryptoServiceInstanceId serviceInstanceID);

  /// @brief Vsome-ip instance for Private Key context proxy.
  ara::crypto::vsomeip::VsomeipInterface<
    ara::crypto::ipccom::cryp::cryobj::privatekey::proxy::PrivateKeyProxy>
    vsomeipInstance_;

  /// @brief Crypto Object context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryobj::privatekey::proxy::PrivateKeyProxy> privateKeyHandler_;

  /// @brief Used to store the crypto service instance id.
  CryptoServiceInstanceId const privateKeyServiceInstanceID_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_PRIVATE_KEY_PROXY_IMPL_H_
