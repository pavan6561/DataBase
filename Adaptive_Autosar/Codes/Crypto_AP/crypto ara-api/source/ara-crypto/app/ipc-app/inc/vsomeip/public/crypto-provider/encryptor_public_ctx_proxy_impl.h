...
// -----------------------------------------------------------------------------------------------------------
// File Name   : encryptor_public_ctx_proxy_impl.h
//
...
#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_ENCRYPTOR_PUBLIC_CTX_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_ENCRYPTOR_PUBLIC_CTX_PROXY_IMPL_H_
...
#include "encryptorpubliccontext_proxy.h"
...
namespace cryp
{
class EncryptorPublicCtxProxyImpl final : public ara::crypto::cryp::EncryptorPublicCtx
{
 public:
  /// @brief Constructor for the EncryptorPublicCtxProxyImpl class.
  /// @param[in] instanceValue CryptoServiceInstance Id for the Asymmetric cipher.
  /// @param[in] encPubCryptoProviderHandler CryptoProviderInterfaceProxy Shared pointer for the Asymmetric
  /// cipher.
  /// @param[in] cp Reference to the crypto provider instance.
  EncryptorPublicCtxProxyImpl(CryptoServiceInstanceId const instanceValue,
  ...
  /// @brief Destructor for the EncryptorPublicCtxProxyImpl class.
  ~EncryptorPublicCtxProxyImpl() noexcept
  {
    encPubCryptoProviderHandler_->StopCryptoProviderService(
      CryptoProviderServiceType::kEncryptorPublicCtx, vsomeipEpcInstance_.serviceInstanceID_);
  }
  ...
  /// @returns Unique pointer to CryptoPrimitiveId class
 ...
 private:
  /// @brief Method to initializing the server-client connection for EncryptorPublicCtxProxy context.
  /// @param[in] serviceInstanceID Crypto service instance id.
  /// @returns true if context is initialized successfully else return false.
  bool Initialize(CryptoServiceInstanceId const serviceInstanceID);

  /// @brief vsome-ip instance for EncryptorPublicCtxProxy class.
  ara::crypto::vsomeip::VsomeipInterface<
    ara::crypto::ipccom::cryp::encryptorpubliccontext::proxy::EncryptorPublicContextProxy>
    vsomeipEpcInstance_;

  /// @brief crypto provider handler
  std::shared_ptr<ara::crypto::ipccom::cryp::cryptoprovider::proxy::CryptoProviderInterfaceProxy>
    encPubCryptoProviderHandler_;

  /// @brief Encryptor Public context proxy handler
  std::shared_ptr<ara::crypto::ipccom::cryp::encryptorpubliccontext::proxy::EncryptorPublicContextProxy>
    encPubProxyHandler_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_ENCRYPTOR_PUBLIC_CTX_PROXY_IMPL_H_sss
