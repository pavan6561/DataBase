...
// -----------------------------------------------------------------------------------------------------------
// File Name   : signature_proxy_impl.h
//
// Purpose     : Signature proxy implementation to forward functional calls from application to ipc-core.
// -----------------------------------------------------------------------------------------------------------

#ifndef CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SIGNATURE_PROXY_IMPL_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SIGNATURE_PROXY_IMPL_H_
...
class SignatureProxyImpl : public ara::crypto::cryp::Signature
{
 public:
  /// @brief Constructor of SignatureProxyImpl class.
  /// @param[in] serviceInstanceID of Crypto service instance id.
  SignatureProxyImpl(const CryptoServiceInstanceId serviceInstanceID) noexcept;

  /// @brief Destructor used to destruct the object of SignatureProxyImpl.
  virtual ~SignatureProxyImpl() noexcept = default;

  /// @brief Method for Get the crypto service instance id.
  /// @returns Crypto service instance id of SignatureProxyImpl.
  CryptoServiceInstanceId GetSignatureServiceInstanceId() const
  {
    return this->signatureServiceInstanceID_;
  }
  ...
  ara::crypto::cryp::CryptoPrimitiveId::AlgId GetHashAlgId() const noexcept override;
 ...
 private:
  /// @brief Method to initializing the server-client connection for SignatureProxyImpl contex.
  /// @param[in] serviceInstanceID Crypto service instance id.
  void Initialize(const CryptoServiceInstanceId serviceInstanceID);

  /// @brief Vsome-ip instance for Signature context proxy.
  ara::crypto::vsomeip::VsomeipInterface<ara::crypto::ipccom::cryp::cryobj::signature::proxy::SignatureProxy>
    vsomeipInstance_;

  /// @brief Crypto Object context proxy handler.
  std::shared_ptr<ara::crypto::ipccom::cryp::cryobj::signature::proxy::SignatureProxy> signatureHandler_;

  /// @brief Used to store the crypto service instance id.
  CryptoServiceInstanceId const signatureServiceInstanceID_;
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_APP_IPC_APP_INC_VSOMEIP_PUBLIC_CRYPTO_PROVIDER_CRYOBJ_SIGNATURE_PROXY_IMPL_H_
