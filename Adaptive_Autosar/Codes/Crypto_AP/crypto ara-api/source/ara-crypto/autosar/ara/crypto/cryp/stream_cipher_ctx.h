...
#ifndef CRYPTO_SOURCE_ARA_CRYPTO_AUTOSAR_ARA_CRYPTO_CRYP_STREAM_CIPHER_CTX_H_
#define CRYPTO_SOURCE_ARA_CRYPTO_AUTOSAR_ARA_CRYPTO_CRYP_STREAM_CIPHER_CTX_H_
...
class StreamCipherCtx : public CryptoContext
{
 public:
  ...
  /// @returns true if the mode can process messages the byte-by-byte (without padding up to the block
  /// boundary)
  /// and false if only the block-by-block (only full blocks can be processed, the padding is mandatory)
  /// @trace SWS_CRYPT_23611
  virtual bool IsBytewiseMode() const noexcept = 0;

  /// @brief Check if the seek operation is supported in the current mode.
  /// @returns true the seek operation is supported in the current mode and false otherwise
  /// @trace SWS_CRYPT_23612
  ...
  /// @param[in] fromBegin the starting point for positioning within the stream: from begin (if true) or from
  ...
  /// the Start() method
  /// @error SecurityErrorDomain::kBelowBoundary if the offset value is incorrect (in context of the
  /// the fromBegin argument), i.e. it points before begin of the stream (note: it is an optional
  /// error condition)
  /// @error SecurityErrorDomain::kInvalidArgument if the offset is not aligned to the required boundary
  /// (see IsBytewiseMode())
  /// @trace SWS_CRYPT_23613
  ...
  /// the block size (see GetBlockSize())
  ...
  /// <tt>out.size() >= (((in.size() + bs * ((CryptoTransform::kEncrypt == GetTransformation().Value())
  /// ? 2 : 1) - 1) / bs) * bs)</tt>
  /// If <tt>(IsBytewiseMode() == true)</tt>  then it @b must be:  <tt>out.size() >= in.size()</tt>
  /// The input and output buffers must not intersect!
  ...
  /// If <tt>(IsBytewiseMode() == false)</tt> then it @b must be:
  /// <tt>bs = GetBlockSize(), </tt>
  /// <tt>out.capacity() >= (((in.size() + bs * ((CryptoTransform::kEncrypt == GetTransformation.Value())
  /// ? 2 : 1) - 1) / bs) * bs)</tt>
  /// If <tt>(IsBytewiseMode() == true)</tt>  then it @b must be:  <tt>out.capacity() >= in.size()</tt>
  /// Usage of this method is mandatory for processing of the last data chunk in block-wise modes!
  /// This method may be used for processing of a whole message in a single call (in any mode)!
  ...
  /// (if true) or for reverse one (if false)
  ...
  /// (taking into account the direction specified by transform) is prohibited by
  /// the "allowed usage" restrictions of provided key object
  /// @trace SWS_CRYPT_23623
...
#endif // CRYPTO_SOURCE_ARA_CRYPTO_AUTOSAR_ARA_CRYPTO_CRYP_STREAM_CIPHER_CTX_H_
