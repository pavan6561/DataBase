...
#ifndef CRYPTO_SOURCE_AUTOSAR_CRYPTO_FILES_ARA_CRYPTO_CRYP_CRYOBJ_CRYPTO_OBJECT_H_
#define CRYPTO_SOURCE_AUTOSAR_CRYPTO_FILES_ARA_CRYPTO_CRYP_CRYOBJ_CRYPTO_OBJECT_H_
...
{
 public:
  /// @brief Constructor for the CryptoObject class.
  /// @param[in] ReadWriteMemRegion containing the actual data to be stored in crypto object.
  CryptoObject(ReadWriteMemRegion val) noexcept
  {
    aralog_.LogDebug() << "In CryptoObject constructor";
    crypValue_.resize(val.size());
    for (size_t index{ 0 }; index < val.size(); index++)
    {
      crypValue_.at(index) = val[index];
    }
    cryptoObjectMemory_ = crypValue_;
  }

  /// @brief Move Constructor for CryptoObject class.
  CryptoObject(CryptoObject&& other) noexcept = default;
  ...
  /// @param other is a parameter of crypto object instance
  ...
  {
    /// @brief Destination type for the crypto object.
    using DstValType = typename ConcreteObject::Uptrc;

    if (object)
    {
      const CryptoObjectType kObjType{ object->GetObjectId().mCOType };
      if (kObjType != ConcreteObject::sObjectType)
      {
        ara::core::ErrorDomain::SupportDataType data{
          (static_cast<ara::core::ErrorDomain::SupportDataType>(kObjType) << 16)
          | static_cast<ara::core::ErrorDomain::SupportDataType>(ConcreteObject::sObjectType)
        };
        return ara::crypto::MakeErrorCode(SecurityErrc::kBadObjectType, data);
      }
    }
  ...
  /// @brief Getter method for Key value.
  /// @return ReadWriteMemRegion containing the Key value.
  virtual ReadWriteMemRegion GetValue() const noexcept
  {
    aralog_.LogDebug() << "In CryptoObject::GetValue()";
    return cryptoObjectMemory_;
  }

  /// @brief Getter method for Key value in vector.
  /// @return vector containing the Key value.
  virtual ara::core::Vector<uint8_t> GetValueInVector() const noexcept
  {
    aralog_.LogDebug() << "In CryptoObject::GetValueInVector()";
    return crypValue_;
  }

  /// @brief setter method for Key value.
  /// @param[in] ReadWriteMemRegion containing the actual data to be stored in crypto object.
  void SetValue(ReadWriteMemRegion objectValue) noexcept
  {
    aralog_.LogDebug() << "In CryptoObject::SetValue()";
    crypValue_.resize(objectValue.size());
    for (size_t index{ 0 }; index < objectValue.size(); index++)
    {
      crypValue_.at(index) = objectValue[index];
    }
  }

 protected:
  /// @brief vector to store the actual data of the crypto object.
  ara::core::Vector<uint8_t> crypValue_{};

  /// @brief ReadWriteMemRegion containing the address of the actual data of the crypto object.
  ReadWriteMemRegion cryptoObjectMemory_{};
...
#endif // CRYPTO_SOURCE_AUTOSAR_CRYPTO_FILES_ARA_CRYPTO_CRYP_CRYOBJ_CRYPTO_OBJECT_H_
